...
def batchnorm_backward(dout, cache):
    ...
    # step 9
    dbeta = np.sum(dout,0) # (D,)
    dgamma_xhat = 1*dout # (N,D)
    
    # step 8 
    dxhat = dgamma_xhat*gamma # (N,D)
    dgamma = np.sum(dgamma_xhat*xhat, 0) #(D,)
    
    # step 7
    divar = np.sum(dxhat*xmu, 0) # (D,)
    ...
    # step 5
    dvar = dsqrtvar*0.5*(1./np.sqrt(var+eps)) # (D,)
    
    # step 4 
    dsq =  1./N * np.ones((N,D)) * dvar # (D,)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_new):
        for j in range(W_new):
            ...
            for k in range(F):
                out[:, k, i, j] = np.sum( w[k, :, :, :] * x_pad_mask, axis = (1,2,3)) + b[k]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_new):
        for j in range(W_new):
            ...
            for k in range(F):
                # print( x_pad_mask.shape, (dout[:,k,i,j].shape), (dout[:,k,i,j])[:, None, None, None].shape)
                dw[k, :, :, :] += np.sum( x_pad_mask * (dout[:,k,i,j])[:, None, None, None], axis = 0)
                
            for n in range(N):
                # print( w.shape, (dout[n,:,i,j].shape), (dout[n,:,i,j])[:, None, None, None].shape)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_new):
        for j in range(W_new):
            x_mask = x[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            max_x_mask = np.max(x_mask, axis=(2,3))
            dx[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += (x_mask==max_x_mask[:,:,None,None]) \
                                                                    * (dout[:,:,i,j])[:,:,None,None]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_temp = x.transpose(0,3,2,1).reshape(N*W*H, C)
    out_temp, cache = batchnorm_forward(x_temp, gamma, beta, bn_param)
    out = out_temp.reshape(N,W,H,C).transpose(0,3,2,1)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_temp = dout.transpose(0,3,2,1).reshape((N*H*W,C))
